...
use BlackCat\Database\Packages\PaymentLogs\Dto\PaymentLogDto as Dto;
use BlackCat\Database\Packages\PaymentLogs\Mapper\PaymentLogDtoMapper as RowMapper;
...
final class PaymentLogRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\PaymentLogs\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\PaymentLogs\Definitions::table());
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\PaymentLogs\Dto\PaymentLogDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\PaymentLogs\Dto\PaymentLogDto|null {
        $row = $this->getByUnique([ 'id' => $id ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\PaymentLogs\Mapper\PaymentLogDtoMapper::fromRow($row);
    }
