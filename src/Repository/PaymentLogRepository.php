...
namespace BlackCat\Database\Packages\PaymentLogs\Repository;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\PaymentLogs\Definitions;
use BlackCat\Database\Packages\PaymentLogs\Criteria;
use BlackCat\Database\Packages\PaymentLogs\ContractRepository;

final class PaymentLogRepository implements ContractRepository {
    ...
    public function insert(array $row): void {
        ...
        $sql = "INSERT INTO payment_logs (".implode(',', $cols).") VALUES (".implode(',', $place).")";
    ...
    public function insertMany(array $rows): void {
        ...
        $sql = "INSERT INTO payment_logs (".implode(',', $cols).") VALUES ".implode(',', $allPlace);
        $this->db->execute($sql, $params);
    }
    ...
    public function upsert(array $row): void {
        ...
        $sqlBase = "INSERT INTO payment_logs (".implode(',', $cols).") VALUES (".implode(',', $place).")";
    ...
    public function updateById(int|string $id, array $row): int {
        ...
        $sql = "UPDATE payment_logs SET ".implode(',', $assign)." WHERE id = :id".$verCond;
    ...
    public function deleteById(int|string $id): int {
        ...
        if ($soft) {
            ...
            return $this->db->execute("UPDATE payment_logs SET $set WHERE id = :id", $params);
        }
        return $this->db->execute("DELETE FROM payment_logs WHERE id = :id", [':id'=>$id]);
    }
    ...
    public function restoreById(int|string $id): int {
        ...
        $set = "$soft = NULL";
        if ($updAt && $updAt !== $soft) { $set .= ", $updAt = CURRENT_TIMESTAMP"; }
        return $this->db->execute("UPDATE payment_logs SET $set WHERE id = :id", [':id'=>$id]);
    }
    ...
    public function exists(string $whereSql = '1=1', array $params = []): bool {
        $sql = "SELECT 1 FROM payment_logs WHERE $whereSql LIMIT 1";
        return (bool)$this->db->fetchOne($sql, $params);
    }

    public function count(string $whereSql = '1=1', array $params = []): int {
        $sql = "SELECT COUNT(*) FROM payment_logs WHERE $whereSql";
    ...
    public function paginate(Criteria $c): array {
        ...
        $total = (int)$this->db->fetchOne("SELECT COUNT(*) FROM payment_logs t $joins WHERE $where", $params);
        $items = $this->db->fetchAll("SELECT t.* FROM payment_logs t $joins WHERE $where ORDER BY $order LIMIT $limit OFFSET $offset", $params);
