 ...
 * - whitelist filtrů: [ 'id', 'payment_id', 'log_at', 'message' ]
 * - whitelist pro LIKE hledání: [ 'message' ]
 */
final class Criteria {
    ...
    public function toSql(bool $viewMode = false): array {
        ...
        // filtry (rovnosti / IN)
        $allowed = array_fill_keys([ 'id', 'payment_id', 'log_at', 'message' ], true);
        ...
        $order = null;
        if ($this->sort) {
            $safeCols = array_fill_keys([ 'id', 'payment_id', 'log_at', 'message' ], true);
            $parts = [];
