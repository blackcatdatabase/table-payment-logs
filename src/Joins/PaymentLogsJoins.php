...
namespace BlackCat\Database\Packages\PaymentLogs\Joins;
...
final class PaymentLogsJoins {
    ...
    /**
     * FK: payment_logs -> payments
     * LEFT JOIN vw_payments AS $as ON $as.id = $alias.payment_id
     * @return array{0:string,1:array}
     */
    public function joinPayments(string $alias = 't', string $as = 'j0'): array {
        [$alias, $as] = $this->assertAliasPair($alias, $as);
        return [' LEFT JOIN vw_payments AS ' . $as . ' ON ' . $as . '.id = ' . $alias . '.payment_id' . ' ', []];
    }
