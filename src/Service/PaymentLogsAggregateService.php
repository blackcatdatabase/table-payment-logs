 ...
 */
final class PaymentLogsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private PaymentLogRepository $paymentLogRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'payment_logs' => $this->paymentLogRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\PaymentLogs\ModuleException("Record {$idStr} not found for locking.");
            }
